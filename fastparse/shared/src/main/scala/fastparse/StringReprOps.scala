...
object StringReprOps extends ReprOps[Char, String] {
  ...
  def errorMessage(input: ParserInput[Char, String], expected: String, idx: Int): String = {
    val locationCode = {
      ...
      val emptyString = ""
      val lastSnippet: String = last.linesIterator.toSeq.headOption.getOrElse(emptyString)
      val firstSnippet: String = first.reverse.linesIterator.toSeq.headOption.getOrElse(emptyString).reverse
